...
text parser(text input, char *str) {
    ...
    for (int i = 0; i < count; i++) {
        ...
        if (ft_toupper(tmp[0][0]) == 'C') {
            ...
            // printf("%d | %d\n",s_line, e_line);
            ret_text.text = center_text(ret_text.text, s_line, e_line, ret_text.rows);
        } else
        if (ft_toupper(tmp[0][0]) == 'L') {
            if(tmp[1] && isdigit(tmp[1][0]))
                s_line = atoi(tmp[1]);
            if(tmp[2] &&isdigit(tmp[2][0]))
                e_line = atoi(tmp[2]);
                ...
                e_line = ret_text.rows;
            ret_text.text = alignl_text(ret_text.text, s_line, e_line, ret_text.rows);
        } else
        if (ft_toupper(tmp[0][0]) == 'R') {
            if(tmp[1] && isdigit(tmp[1][0]))
                s_line = atoi(tmp[1]);
            if(tmp[2] && isdigit(tmp[2][0]))
                e_line = atoi(tmp[2]);
                ...
                e_line = ret_text.rows;
            ret_text.text = alignr_text(ret_text.text, s_line, e_line, ret_text.rows);
        } else
        if (ft_toupper(tmp[0][0] == 'J')) {
            if(tmp[1] && isdigit(tmp[1][0]))
                s_line = atoi(tmp[1]);
            if(tmp[2] && isdigit(tmp[2][0]))
                e_line = atoi(tmp[2]);
...
// //     char **car;
// // car = (char **)malloc(sizeof(char *) * 50);
// //       for (int i = 0; i < 50; i ++)
// //         car[i] = (char *)malloc(sizeof(char) * 1000);

// //     char str[80] = "W 50, C 0 0, L 1";
